...
WindowManagement::WindowManagement()
{
    ...
    this->entity_handler = new EntityHandler();
...
bool WindowManagement::system_init()
{
    ...
    if(this->entity_handler->rov==NULL)
    {
        this->entity_handler->rov = new ROV();
    }

    if(this->entity_handler->turtle==NULL)
    {
        this->entity_handler->turtle = new TURTLE();
    }

    if(this->entity_handler->rocks.empty())
    {
        for(int i=0; i < 100 ; i++)
        {
            this->entity_handler->rocks.push_back(new ROCK());
        }
    }

    if(this->entity_handler->straws.empty())
    {
        for(int i=0; i < 10 ; i++)
        {
            this->entity_handler->straws.push_back(new STRAW());
        }
    }
...
void WindowManagement::mainloop()
{
    ...
    {
        /* Render here */
        this->entity_handler->rov -> move(
            this->entity_handler->turtle,
            this->entity_handler->rocks,
            this->pressing,
            this->lit2_angle,
            this->lit2_angle_y,
            this->lit2_exponent,
        ...
        if(this->entity_handler->rov->catching_turtle)
            this->entity_handler->rov -> move_turtle(this->entity_handler->turtle);

        this->entity_handler->turtle -> move(this->entity_handler->rocks);

        for(auto &i: this->entity_handler->straws)
            i->move();
